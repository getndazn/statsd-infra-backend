...
var collectMetrics = function nriCollectMetrics(rawMetrics, rules) {
  ...
  // tags support
  global_tags = rawMetrics.global_tags || {};
  ...
  var evalRule = function evalRule(rule, metricName, value) {
    ...
    if (found) {
      if (validateKeyWithSchema(metricName, rule.metricSchema)) {
        ...
        // Append tags to attributes matching key
        Object.keys(global_tags).forEach(function(metric_key){
          if(metricName.includes(metric_key)) {
            global_tags[metric_key].forEach(function(tagObject){
              tag = Object.keys(tagObject)[0];
              //fields[tag] = tagObject[tag];
              data[entityId].metrics[idx][tag] = tagObject[tag];
            })
          }
        }); 
